...
**架构模式**：`context → gap → approach → result → implication → boundary`
...
**架构模式**：`field scale → bottleneck → prior attempts → unresolved gap → present study`

| 要素 | 在论文中的位置 |
|------|--------------|
| **field scale** | 问题重述开头段 |
| **bottleneck** | 问题重述结尾 |
| **prior attempts** | 问题分析第一段 |
| **unresolved gap** | 问题分析第二段 |
| **present study** | 问题分析结尾段 |
...
**架构模式**：`central advance → evidence meaning → relation to prior work → constraints → future use`
