...
from real_modelize_agent.tools.file_tools import read_text_lossy
from real_modelize_agent.tools.notepad_tool import NOTEPAD_FILE, read_notepad
...
def build_layered_memory(state: dict[str, Any], *, node: str = "graph") -> dict[str, Any]:
    ...
    working_memory = {
        ...
        "problem_json_summary": _short_text(
            json.dumps(state.get("problem_json", {}), ensure_ascii=False, default=str),
            1200,
        ),
...
def persist_history_summary(state: RuntimeState, summary: str) -> dict[str, Any]:
    ...
    content = f"# RealModelize History Summary\n\n_Updated: {timestamp}_\n\n{summary.strip()}\n"
