...
def shorten(value: Any, limit: int = 260, keep_tail: bool = False) -> str:
...
def summarize_custom_event(event: dict[str, Any]) -> EventSummary:
    ...
    if event_type == "problem_decision":
        ...
        title = problem.get("title") if isinstance(problem, dict) else ""
        body = (
            f"detected: {detected}\n"
            f"title: {title or '(none)'}\n"
            f"questions: {problem.get('ques_count', 0) if isinstance(problem, dict) else 0}\n"
            f"reason: {shorten(event.get('reason', ''), 400)}"
        )
        return EventSummary("Problem Decision", body, "intent", "green" if detected else "yellow")
    if event_type == "paper_compile":
        ...
        if isinstance(status, dict):
            ...
            body = f"phase: {phase}\n{shorten(status, 400)}"
        return EventSummary("Paper Compile", body, "paper", "green" if isinstance(status, dict) and status.get("compile_ok") else "yellow")
    ...
    if event_type == "handoff":
        return EventSummary(
            ...
            shorten(event.get("task") or event.get("instruction") or "", 500),
            "handoff",
            "yellow",
        )
    if event_type == "handoff_result":
        return EventSummary(
            ...
            shorten(event.get("summary") or event.get("result") or "", 600),
            "handoff",
            "green",
        )
    if event_type == "research_file":
        return EventSummary(
            ...
            (
                ...
                f"queries: {len(event.get('queries', []) if isinstance(event.get('queries'), list) else [])}"
            ),
            "search",
            "cyan",
        )
    if event_type == "figure_check":
        ...
        return EventSummary(
            ...
            (
                f"passed: {passed}\n"
                f"{shorten(event.get('detail', ''), 400)}"
            ),
            "figure",
...
def summarize_graph_event(payload: dict[str, Any]) -> EventSummary:
    ...
    if node == "planner":
        base = _summarize_plan(update, "Planner")
        extra = []
        if update.get("modeler_plan"):
            extra.append(f"modeler_plan: {'ok' if isinstance(update.get('modeler_plan'), dict) else '?'}")
        if update.get("figures"):
            extra.append(f"figures: {len(update.get('figures'))}")
        if update.get("paper_path"):
            extra.append(f"paper: {update.get('paper_path')} compile_ok={update.get('paper_compile_ok')}")
        if extra:
