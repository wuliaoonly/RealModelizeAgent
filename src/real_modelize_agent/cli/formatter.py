...
def print_custom_event(event: dict[str, Any]) -> None:
    ...
    if event_type == "problem_decision":
        ...
        return
    if event_type == "paper_compile":
        render_paper_compile(event)
        return
    ...
    if event_type == "search_summary":
        ...
        return
    if event_type == "research_file":
        render_research_file(event)
        return
    if event_type == "figure_check":
        render_figure_check(event)
        return
...
def render_problem_decision(event: dict[str, Any]) -> None:
    ...
    style = "green" if detected else "yellow"
    lines = [f"detected: {detected}", f"reason: {_shorten(event.get('reason', ''), 600)}"]
    problem = event.get("problem_json")
    if isinstance(problem, dict) and problem:
        lines.append(f"title: {problem.get('title', '')}")
        lines.append(f"questions: {problem.get('ques_count', '')}")
        for key in sorted(problem):
            if key not in {"title", "background", "ques_count"}:
                lines.append(f"  {key}: {_shorten(str(problem[key]), 240)}")
    console.print(Panel("\n".join(lines), title="Problem Decision", border_style=style, box=box.ROUNDED))


def render_paper_compile(event: dict[str, Any]) -> None:
    phase = event.get("phase", "")
    status = event.get("status")
    title = f"Paper Compile · {event.get('node', 'writerAgent')} · {phase}"
    if not isinstance(status, dict):
        console.print(Panel(_shorten(event, 600), title=title, border_style="magenta", box=box.ROUNDED))
        return
    lines = [
        f"tex: {status.get('paper_tex', '') or '(none)'}",
        f"pdf: {status.get('paper_pdf', '') or '(none)'}",
        f"compile_ok: {status.get('compile_ok', False)}",
        f"chars: {status.get('tex_chars', 0)}",
    ]
    style = "green" if status.get("compile_ok") else "yellow"
    console.print(Panel("\n".join(lines), title=title, border_style=style, box=box.ROUNDED))


def render_figures(figures: list[Any]) -> None:
    items = [str(path) for path in figures]
    table = Table(box=box.SIMPLE_HEAVY, header_style="bold")
    table.add_column("#", no_wrap=True)
    table.add_column("Figure path")
    for index, path in enumerate(items, start=1):
        table.add_row(str(index), path)
    console.print(Panel(table, title=f"Figures ({len(items)})", border_style="cyan", box=box.ROUNDED))


def render_research_file(event: dict[str, Any]) -> None:
    queries = event.get("queries") if isinstance(event.get("queries"), list) else []
    lines = [
        f"path: {event.get('path', '') or '(none)'}",
        f"sources: {event.get('sources', 0)}",
        f"queries: {', '.join(str(q) for q in queries) or '(none)'}",
    ]
    console.print(Panel("\n".join(lines), title="Research File", border_style="cyan", box=box.ROUNDED))


def render_figure_check(event: dict[str, Any]) -> None:
    passed = bool(event.get("passed"))
    style = "green" if passed else "yellow"
    console.print(
        Panel(
            _shorten(event.get("detail", ""), 600),
            title=f"Figure Check · {event.get('node', 'writerAgent')} · {'PASS' if passed else 'FAIL'}",
            border_style=style,
            box=box.ROUNDED,
        )
    )
...
def render_modeler_plan(plan: Any) -> None:
    if not isinstance(plan, dict):
        console.print(Panel(_shorten(plan, 900), title="Modeler Plan", border_style="cyan", box=box.ROUNDED))
        return
    table = Table(box=box.SIMPLE_HEAVY, header_style="bold")
    ...
    for key, value in plan.items():
        if isinstance(value, (dict, list)):
            table.add_row(str(key), _shorten(json.dumps(value, ensure_ascii=False, default=str), 480))
        else:
            table.add_row(str(key), _shorten(str(value), 480))
    console.print(Panel(table, title="Modeler Plan", border_style="cyan", box=box.ROUNDED))
