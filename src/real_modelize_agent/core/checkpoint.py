...
CHECKPOINT_ROOT = Path(".real-modelize") / "checkpoints"
...
class CheckpointManager:
    def __init__(self, runtime: Any, task: str = "") -> None:
        ...
        self._last_snapshot_node: str | None = None
    ...
    def save(
        ...
        *,
        ...
        snapshot_needed = status != "running" or latest_node != self._last_snapshot_node
        if snapshot_needed:
            git_commit, git_error = snapshot_workspace_git(self.workspace, self.root)
            self._last_snapshot_node = latest_node
        else:
            git_commit, git_error = git_head(self.workspace, self.root / GIT_DIR), None
...
def build_light_resume_inputs(runtime: Any, *, task: str | None = None, max_attempts: int = 3) -> dict[str, Any]:
    ...
    if not resume_task:
        resume_task = "Continue the interrupted RealModelize task from the checkpoint."
    else:
        resume_task = f"Continue this RealModelize task from the checkpoint: {resume_task}"
...
def normalize_resume_task(task: str) -> str:
    prefixes = (
        "Continue this RealModelize task from the checkpoint:",
        "Continue the interrupted RealModelize task from the checkpoint:",
    )
...
def state_summary(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_commands": json_safe(state.get("execution_commands", [])),
        "figure_audit": json_safe(state.get("figure_audit", {})),
        "passed": state.get("passed"),
        "attempts": state.get("attempts", 0),
        "problem_json": trim_text(state.get("problem_json", {}), 2000),
        "problem_artifacts": json_safe(state.get("problem_artifacts", {})),
...
def snapshot_workspace_git(workspace: Path, root: Path) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        _git(workspace, git_dir, ["config", "user.name", "RealModelize Checkpoint"])
        _git(workspace, git_dir, ["config", "user.email", "real_modelize_agent-checkpoint@example.local"])
        _ensure_git_excludes(git_dir)
        _purge_excluded_git_paths(workspace, git_dir)
...
def resume_command(workspace: Path) -> str:
    return f"uv run real-modelize --resume {shlex.quote(str(workspace))}"
...
def should_skip_workspace_path(rel: Path) -> bool:
    parts = rel.parts
    if len(parts) >= 2 and parts[0] == ".real-modelize" and parts[1] == "checkpoints":
        return True
    skip_names = {".git", ".venv", "venv", "node_modules", "__pycache__", ".pytest_cache", ".test-tmp"}
    secret_names = {".env", ".real-modelize.env", "credentials.json", "secrets.json"}
    name = rel.name.lower()
    return (
...
def _ensure_git_excludes(git_dir: Path) -> None:
    ...
    patterns = [
        ...
        ".pytest_cache/",
        ".test-tmp/",
        # Input/output binaries remain in the workspace; checkpoint Git stores
        # source/state only to avoid duplicating large competition attachments.
        "*.xlsx",
        "*.xls",
        "*.pdf",
        "*.png",
...
def _purge_excluded_git_paths(workspace: Path, git_dir: Path) -> None:
    """Remove secrets/large excluded artifacts from older checkpoint indexes only."""
    try:
        listed = _git(workspace, git_dir, ["ls-files", "-z"]).stdout
    ...
    for raw in listed.split("\0"):
        if not raw:
            continue
        rel = Path(raw)
        if should_skip_workspace_path(rel) or rel.suffix.lower() in {
            ".xlsx", ".xls", ".pdf", ".png", ".jpg", ".jpeg", ".svg"
        }:
            subprocess.run(
                ["git", f"--git-dir={git_dir}", f"--work-tree={workspace}", "rm", "--cached", "--ignore-unmatch", "--", raw],
                cwd=workspace,
                check=False,
                text=True,
                capture_output=True,
