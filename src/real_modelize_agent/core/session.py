...
from real_modelize_agent.core.checkpoint import workspace_manifest


SESSION_ROOT = Path(".real-modelize") / "session"
...
def build_session_context(workspace: Path, session: dict[str, Any] | None = None) -> str:
    ...
    files = [
        ...
        if item.get("type") == "file" and not str(item.get("path", "")).startswith(".real-modelize/session/")
    ][:30]
    recent_turns = [
        {
            "turn": turn.get("turn"),
